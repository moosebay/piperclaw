...
function handlePiperRequest(
  ...
  // Resolve the piper directory from the control UI build output
  ...
  // Strip /piper prefix to get the relative path
...
export function createGatewayHttpServer(opts: {
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      // Piper dashboard — served at /piper as a separate SPA
      requestStages.push({
        name: "piper-dashboard",
        run: () => handlePiperRequest(req, res, requestPath, controlUiRoot),
      });
